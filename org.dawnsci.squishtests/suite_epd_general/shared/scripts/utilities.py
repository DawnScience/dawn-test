source(findFile("scripts", "pydev_python.py"))
...
def verifyAndClearErrorLog(reportErrors=True):
    ''' Verifies that the error log is empty and clears it if not '''
    errors = getErrorItems()
    if errors.length > 0:
        firstError = errors.at(0)
        if (errors.length==1):
            data = firstError.data.toString()
            if ("Warning"==data):
                return
        if (reportErrors):
            test.fail("There are errors in error log view (if this test fails, see console output as -consoleLog is on)")
        mouseClick(waitForObject(":Delete Log_ToolItem"))
        clickButton(waitForObject(":Confirm Delete.OK_Button"))
    else:
